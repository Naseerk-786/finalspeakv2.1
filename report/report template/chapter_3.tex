...
\begin{enumerate}
    \item \textbf{Sub-2ms Inference Latency:} Evaluating a single static frame eliminated the 30-frame video buffer, reducing inference latency from >500ms down to **$<1.8\text{ ms}$**.
...
To train our classifier on a comprehensive baseline, we developed an automated multi-threaded harvesting pipeline (\texttt{prototype/download\_and\_extract\_isl\_letters.py}) that harvested and merged 129,773 raw sign alphabet images across three public repositories: Kaggle ISL, Kaggle ASL, and GitHub ISL.
