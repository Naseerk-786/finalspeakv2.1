...
\section{The Breakthrough: Closing the Two-Way Conversational Loop}
During our early user testing, we observed a fundamental limitation of almost all sign language apps: they are \textbf{one-way megaphones}. They allow the deaf user to speak, but offer no mechanism for the deaf user to understand the hearing partner's verbal reply without juggling separate speech-to-text apps on a phone.

We engineered a \textbf{Bidirectional Conversational Loop} (illustrated in Figure~\ref{fig:twoway_loop}):

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/fig_07_twoway_loop.png}
    \caption{SignSpeak Universal Bidirectional Conversational Loop: Forward sign-to-speech loop (green) paired with reverse microphone speech-to-sign loop (blue) via Whisper AI and ISL letter badge rendering.}
    \label{fig:twoway_loop}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{SpeechToTextThread} streams 16kHz audio in the background and transmits packaged audio frames to Groq's \texttt{whisper-large-v3-turbo} endpoint, returning transcripts in \textbf{$<200\text{ ms}$}.
...
In Version 3.0/3.1, we redesigned the desktop interface around a calm, camera-first assistive workspace (wireframed in Figure~\ref{fig:ui_wireframe}):

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/fig_11_ui_wireframe_hierarchy.png}
    \caption{SignSpeak Universal Workspace UI/UX Wireframe: 60\% Hero Camera Viewport with status overlays, accessible touch controls, predictive autocomplete strip, and collapsible progressive drawer.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/fig_06_latency_breakdown.png}
    \caption{End-to-End System Execution Latency Budget Breakdown: Measured latency across each pipeline stage, totaling just 23.4 ms (well below the 50 ms interactive ceiling).}
    \label{fig:latency_breakdown}
\end{figure}

As shown in Figure~\ref{fig:latency_breakdown}, total end-to-end system latency from camera acquisition to speech playback is \textbf{$23.4\text{ ms}$}, well below the 50ms human perception ceiling.
